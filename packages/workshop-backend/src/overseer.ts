...
class OverseerClientInterface extends RpcTarget implements Overseer {
  ...
  async newAiModelGatekeeper(modelId: string): Promise<GatekeeperClient<any>> {
    ...
    if (SUBSCRIPTION_PROVIDERS.has(chatMeta.aiModel!.config.provider)) {
      // A binding would freeze the sign-in's access token in the gatekeeper's props and let a
      // gadget drive a personal plan programmatically; both are wrong, so these stay chat-only.
      throw new Error("Models signed in through a personal subscription can't be bound to " +
          "gadgets. Bind an API-key model instead.");
    }
