...
import { PublicApi, AuthenticatedApi, Overseer, GadgetMetadataWithTimestamps, AiChatAuthorInfo, AiModelConfig, AiGatewayInfo, AiModelProvider, ChatGptDeviceLogin, ChatGptDeviceLoginStatus, ChatGptSubscriptionInfo, ConnectedAccountsSubscriber, ConnectedAccountsFilter, GatekeeperVendorFilter, ObserverConfigCallback, BlueprintLibrarySummary, BlueprintPublicInfo, BlueprintUserSummary, BlueprintBindingAssignment, AgentSpawnerConfig, WorkpieceId, BLUEPRINT_SCREENSHOT_PATH_PREFIX, BLUEPRINT_SCREENSHOT_R2_PREFIX, blueprintScreenshotUrl, ServerConfig, CloudflareUsageInfo, CloudflareAccountOption, LoginAttempt, GatekeeperAppInfo, AdminApi, GatekeeperVendorInfo, OutputFormatOffer, ListOutputsResult, createOpenGadgetError, getOpenGadgetErrorCode, OPEN_GADGET_ERROR_CODES, AUTH_ERROR_CODES, createAuthError } from '@gadgets/workshop-shared/api';
...
class AuthenticatedApiImpl extends RpcTarget implements AuthenticatedApi {
  ...
  startChatGptDeviceLogin(): Promise<ChatGptDeviceLogin> {
    return this.#user.startChatGptDeviceLogin();
  }
  pollChatGptDeviceLogin(): Promise<ChatGptDeviceLoginStatus> {
    return this.#user.pollChatGptDeviceLogin();
  }
  getChatGptSubscription(): Promise<ChatGptSubscriptionInfo | null> {
    return retryOnDoReset(() => this.#user.getChatGptSubscription());
  }
  disconnectChatGpt(): Promise<void> {
    return this.#user.disconnectChatGpt();
  }
